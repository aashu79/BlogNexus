...
<body>
    ...
    <div class="content">
        <div class="dashboard-header">
            ...
            <div>
                <p>Current Date and Time (UTC): 
                    <fmt:parseDate value="${currentDateTime}" pattern="yyyy-MM-dd'T'HH:mm:ss" var="parsedDate" type="both" />
                            ...
                            <c:otherwise>
                                <c:forEach items="${users}" var="user">
                                    <tr>
                                            ...
                                            <div class="user-avatar">
                                                <c:choose>
                                                    <c:when test="${not empty user.profilePictureUrl}">
                                                        <img src="${contextPath}${user.profilePictureUrl}" alt="${user.firstName} ${user.lastName}">
                                                    </c:when>
                                                    <c:otherwise>
                                        ...
                                        <td>
                                            <span class="role-badge ${user.userRole == 'admin' ? 'admin' : 'user'}">${user.userRole}</span>
                                        </td>
                                        <td>
                                            <div class="action-buttons">
                                                <c:choose>
                                                    <c:when test="${user.userId == currentUser.userId}">
                                                        <span class="action-disabled" title="Cannot delete yourself">
                                                            <i class="fas fa-trash-alt"></i> Delete
                                                        </span>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <form method="post" action="${contextPath}/admin/users/delete" class="delete-form" onsubmit="return confirmDelete('${user.firstName} ${user.lastName}');">
