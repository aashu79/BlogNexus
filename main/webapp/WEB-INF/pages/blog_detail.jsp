    ...
    <main class="blog-main">
        <div class="container">
                    ...
                    <div class="author">
                        <c:choose>
                            <c:when test="${not empty authorProfilePictureUrl}">
                                <img src="${pageContext.request.contextPath}${authorProfilePictureUrl}" 
            ...
            <div class="featured-image-wrapper">
                <div class="featured-image">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <img src="https://images.unsplash.com/photo-1465146344425-f00d5f5c8f07?w=1200" 
                                 alt="${blog.title}">
                        </c:otherwise>
                    </c:choose>
                </div>
            ...
            <div class="blog-actions">
                <div class="blog-actions-left">
                    <c:if test="${isLoggedIn}">
                        <form action="${pageContext.request.contextPath}/blog/favourite" method="post" class="inline-form">
                            <input type="hidden" name="blogId" value="${blog.blogId}">
                            <input type="hidden" name="action" value="${isFavourite ? 'remove' : 'add'}">
                            <button type="submit" class="action-btn favorite-btn ${isFavourite ? 'active' : ''}">
                ...
                <div class="blog-actions-right">
                    <c:if test="${isLoggedIn}">
                        <form action="${pageContext.request.contextPath}/blog/like" method="post" class="inline-form">
                            <input type="hidden" name="blogId" value="${blog.blogId}">
                            <input type="hidden" name="action" value="${isLiked ? 'remove' : 'add'}">
                            <button type="submit" class="action-btn like-btn ${isLiked ? 'active' : ''}">
            ...
            <section class="comments-section">
                <h2 class="section-title">Comments <span class="comment-count">(${commentCount})</span></h2>
                ...
                <c:if test="${isLoggedIn}">
                    <div class="add-comment">
                        <div class="comment-user">
                            <c:choose>
                                <c:when test="${not empty currentUser.profilePictureUrl}">
                                    <img src="${pageContext.request.contextPath}${currentUser.profilePictureUrl}" 
                        ...
                        <form class="comment-form" action="${pageContext.request.contextPath}/blog/comment" method="post">
                            <input type="hidden" name="blogId" value="${blog.blogId}">
                            <textarea name="commentContent" placeholder="Share your thoughts..." rows="3" required></textarea>
                            ...
                            <c:forEach items="${comments}" var="comment">
                                <div class="comment">
                                    <div class="comment-header">
                                        <div class="comment-user">
                                            <c:choose>
                                                <c:when test="${not empty comment.commenterProfilePictureUrl}">
                                                    <img src="${pageContext.request.contextPath}${comment.commenterProfilePictureUrl}" 
                                            ...
                                            <div>
                                                ...
                                                <span class="comment-date">
                                                    <fmt:parseDate value="${comment.commentedAt}" pattern="yyyy-MM-dd'T'HH:mm:ss" var="parsedDate" type="both" />
                                                    <fmt:formatDate value="${parsedDate}" pattern="MMMM d, yyyy • HH:mm" />
                                                </span>
                                            </div>
                                        </div>
                                    </div>
