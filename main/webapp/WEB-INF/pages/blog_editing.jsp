...
<body>
	...
	<div class="blog-container">
		...
		<form action="${contextPath}/user/blog/edit" method="post"
			...
			<div class="form-group">
				<label for="title">Blog Title</label> <input type="text" id="title"
					name="title" placeholder="Enter your blog title" required
					value="${blog.title}">
			</div>
			...
			<div class="form-group">
				...
				<textarea id="content" name="content"
					placeholder="Write your blog here..." required>${blog.content}</textarea>
			</div>

			<div class="form-group">
				<label for="thumbnail">Update Thumbnail Image (optional)</label> <input
			...
			<div class="form-group">
				<label for="image">Update Featured Image (optional)</label> <input
			...
			<div class="form-group">
				<label for="genre">Genre</label> <select id="genre" name="genre"
					...
					<option value="Technology"
						${blog.genre eq 'Technology' ? 'selected' : ''}>Technology</option>
					...
					<option value="Fashion"
						${blog.genre eq 'Fashion' ? 'selected' : ''}>Fashion</option>
					<option value="Business"
						${blog.genre eq 'Business' ? 'selected' : ''}>Business</option>
					<option value="Entertainment"
						${blog.genre eq 'Entertainment' ? 'selected' : ''}>Entertainment</option>
					<option value="Sports" ${blog.genre eq 'Sports' ? 'selected' : ''}>Sports</option>
					<option value="Education"
						${blog.genre eq 'Education' ? 'selected' : ''}>Education</option>
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			form.addEventListener('submit', function(event) {
				...
				if (!blogIdInput.value) {
					...
					console.log('Submitting form with blog ID: '
							+ blogIdInput.value);
				}
			});
						...
						function(e) {
							if (e.target.files && e.target.files[0]) {
								...
								reader.onload = function(e) {
									document
											.querySelector('#thumbnail + .current-file + .image-preview img').src = e.target.result;
								}
				...
				.addEventListener(
						...
						function(e) {
							if (e.target.files && e.target.files[0]) {
								...
								reader.onload = function(e) {
									document
											.querySelector('#image + .current-file + .image-preview img').src = e.target.result;
								}
								reader.readAsDataURL(e.target.files[0]);
							}
						});
