...
public class Blog_EditingController extends HttpServlet {
	...
	/**
	 * Handles GET requests for displaying blog edit form URL format:
	 * /user/blog/edit/{blogId}
	 * 
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (blog == null) {
				...
				redirectionUtil.urlRedirectWithMessage(request, response, "/", "error", "Blog not found");
				return;
			}
			...
			if (blog.getCreatedBy() != user.getUserId() && !"admin".equals(user.getUserRole())) {
				...
				redirectionUtil.urlRedirectWithMessage(request, response, "/user/profile", "error",
			...
			e.printStackTrace();
			redirectionUtil.urlRedirectWithMessage(request, response, "/", "error",
					"An error occurred while loading the blog");
		}
	}

	/**
	 * Handles POST requests for updating blogs This method handles form submissions
	 * from the edit page.
	 * 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (blogIdParam == null || blogIdParam.trim().isEmpty()) {
			redirectionUtil.urlRedirectWithMessage(request, response, "/user/profile", "error",
					"Blog ID is required for updates");
			return;
		}
