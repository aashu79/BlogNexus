...
public class UserFavouriteService {
    ...
    public List<BlogModel> getFavouriteBlogsByUserId(int userId) {
        ...
        String query = "SELECT b.*, u.first_name, u.last_name, u.profile_picture FROM blog b " +
                       ...
                       "JOIN favourite f ON b.blog_id = f.blog_id " +
                       "WHERE f.user_id = ? " +
                       "ORDER BY f.favourite_id DESC";  // Show newest favourites first
    ...
    public void processRemoveFavourite(HttpServletRequest req, HttpServletResponse res) {
        try {
            ...
            if (session == null || session.getAttribute("user") == null) {
                redirectionUtil.urlRedirectWithMessage(req, res, "/login", "error", "Please login to manage favourites");
                return;
            }
            ...
            try {
                ...
                if (success) {
                    redirectionUtil.urlRedirectWithMessage(req, res, "/user/favourite", "success", "Blog removed from favourites");
                } else {
                    redirectionUtil.urlRedirectWithMessage(req, res, "/user/favourite", "error", "Failed to remove blog from favourites");
                }
