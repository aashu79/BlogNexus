...
public class RegisterService {
	...
	/**
	 * Initializes the registration service with required utilities and database
	 * connection.
	 */
	...
	/**
	 * Registers a new user with provided details from request. Validates input
	 * data, processes profile image, and stores user in database.
	 * 
	...
	public void addUser(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		ValidationResult result = ValidationUtils.verifyDetails(firstName, lastName, email, phone, country, password,
				confirmPassword, profilePicture);
		...
		try (PreparedStatement insertStmt = DbConnection.prepareStatement(insertQuery)) {
			...
			e.printStackTrace();
			redirectionUtil.redirectWithMessage(req, res, "register.jsp", "error",
