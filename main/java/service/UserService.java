...
public class UserService {
    ...
    public void createUser(HttpServletRequest req, HttpServletResponse res) 
        ...
        // Check if email already exists first (since this is not checked in verifyDetails)
        if (isEmailExists(email)) {
            redirectionUtil.urlRedirectWithMessage(req, res, "/admin/users/add", "error", "Email address already exists");
            return;
        }
        
        // Validate user input using existing verifyDetails method
        ValidationResult validationResult = ValidationUtils.verifyDetails(
                firstName, lastName, email, phone, country, 
                password, confirmPassword, profilePicture);
                
        if (!validationResult.isSuccess()) {
            System.err.println("Validation failed: " + validationResult.getMessage());
            redirectionUtil.urlRedirectWithMessage(req, res, "/admin/users/add", "error", validationResult.getMessage());
            return;
        }
        
        // Validate role (not included in verifyDetails)
        if (ValidationUtils.isNullOrEmpty(role) || (!role.equals("admin") && !role.equals("user"))) {
        ...
        if (profilePicture != null && profilePicture.getSize() > 0) {
            ...
            if (!isUploaded) {
                ...
                redirectionUtil.urlRedirectWithMessage(req, res, "/admin/users/add", "error", "Failed to upload profile picture");
                return;
            }
        }
        ...
        try (PreparedStatement stmt = dbConnection.prepareStatement(insertQuery)) {
            ...
            if (rowsAffected > 0) {
                ...
                redirectionUtil.urlRedirectWithMessage(req, res, "/admin/users", "success", "User created successfully!");
            } else {
            ...
            redirectionUtil.urlRedirectWithMessage(req, res, "/admin/users/add", "error", "Database error: " + e.getMessage());
